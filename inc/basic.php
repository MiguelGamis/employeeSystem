...
function get_redirect_url($page)
{
    ...
    if($dataMgr->groupID)
    {
        ...
        if($PRETTYURLS)
        {
            return $prefix.$dataMgr->groupName."/".$page;
        }
        else
        {
...
function boxify($dataArray, $type, $item)
{
	$html = "";
	if(empty($dataArray))
	...
	else
	{
		...
		foreach($dataArray as $data)
		{
			...
			if(is_array($value))
			{
				$html .= "<td><table><ul style='list-style-type: none;'>";
				foreach($value as $subvalue)
					$html .= "<li>$subvalue</li>";
				$html .="</ul></table></td>";
			}
            elseif($field == 'image'){
            	$html .= "<td><img src='data:image;base64,$value'/></td>";
            }
			else {
				$html .= "<td>$value</td>";
			}
