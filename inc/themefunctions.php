...
function get_default_menu_items()
{
    ...
    if($authMgr->isLoggedIn())
    {
        ...
        if($dataMgr->isAllSeeing($USERID))
        {
            $menu[] = array("name" => "Become Other Users", "link" => get_redirect_url("becomeotherusers.php?action=select"));
        }
        //Are they already hiding as a student?
        else if(array_key_exists('oldAllSeeing', $_SESSION))
        {
            $menu[] = array("name" => "Return to Original View", "link" => get_redirect_url("becomeotherusers.php?action=return"));
        }
        $menu[] = array("name" => "Logout", "link" => get_redirect_url("logout.php"));
    }
    return $menu;
}
